    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="bg-card rounded-xl border p-4 flex items-center gap-3">
          ...
          <div><p className="text-2xl font-bold text-foreground">{stats.total}</p><p className="text-xs text-muted-foreground">Total vendeurs (global)</p></div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-card rounded-xl border p-4 flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-emerald-50 text-emerald-600"><CheckCircle2 className="w-5 h-5" /></div>
            <div><p className="text-2xl font-bold text-emerald-600">{stats.actifs}</p><p className="text-xs text-muted-foreground">Actifs ({periodes.find(p => p.key === periode)?.label})</p></div>
          </div>
          <div className="bg-card rounded-xl border p-4 flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-red-50 text-red-600"><XCircle className="w-5 h-5" /></div>
            <div><p className="text-2xl font-bold text-red-600">{stats.inactifs}</p><p className="text-xs text-muted-foreground">Inactifs ({periodes.find(p => p.key === periode)?.label})</p></div>
          </div>
        </div>
      </div>
